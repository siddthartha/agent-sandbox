...
- `claude` with `claude.Dockerfile`: Claude Code in the locally built
...
- Keep the launchers thin: a `docker run` wrapper with the mounts and flags it
  needs, nothing else.
- Any change to a Dockerfile, `build.sh` or a launcher needs a rebuild and the
  check described in the `sandbox-rebuild` skill.
